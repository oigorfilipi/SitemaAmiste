...
function CountRowsEditor({ groupId, rows, onChangeAsset, onChangeQuantity }) {
  ...
  return (
    <div className="max-h-[46vh] space-y-3 overflow-y-auto pr-1">
      {rows.map((row, index) => (
        <section className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-4" key={row.itemId}>
          ...
          {isMachines && Number(row.quantity || 0) > 0 ? (
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              {row.assets.map((asset, assetIndex) => (
                <div className="rounded-2xl border border-zinc-200 bg-white p-3" key={`${row.itemId}_${assetIndex}`}>
  ...
  return (
    ...
    >
      <div className="space-y-5">
        <div className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-zinc-200 bg-zinc-50/80 p-4">
        ...
        <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <div className="min-w-[860px]">
              <div className="grid grid-cols-[1.4fr_120px_180px_170px_120px] gap-4 border-b border-zinc-100 bg-zinc-50/70 px-4 py-3 text-xs font-black uppercase text-amiste-gray/60">
                <span>Produto</span>
              ...
              {rows.length ? rows.map((row) => {
                ...
                return (
                  <div className="border-b border-zinc-100 transition hover:bg-amiste-red/5 last:border-b-0" key={row.itemId}>
                    <div className="grid grid-cols-[1.4fr_120px_180px_170px_120px] items-center gap-4 px-4 py-4">
                      ...
                      <div className="text-right">
                        {isMachines ? (
                          <Button className="h-8 w-[72px] px-3 text-xs" variant="secondary" onClick={() => setExpandedId(expanded ? "" : row.itemId)}>
                    ...
                    {isMachines && expanded ? (
                      <div className="grid grid-cols-1 gap-2 px-4 pb-4 md:grid-cols-2">
                        {row.assets.map((asset, index) => (
                          <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-3" key={`${row.itemId}_${index}`}>
          ...
          {activeGroup === "machines" && Number(editQuantity || 0) > 0 ? (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {editAssets.map((asset, index) => (
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-3" key={`edit_asset_${index}`}>
