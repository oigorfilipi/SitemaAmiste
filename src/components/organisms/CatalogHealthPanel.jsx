    ...
    <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition duration-300 hover:shadow-amiste-soft">
      ...
      {alerts.length ? (
        <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
          {alerts.map((item) => (
            <article className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-3" key={item.id}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-xl bg-amiste-yellow/40 px-2.5 py-1 text-xs font-black uppercase text-yellow-900">
                  {item.healthLabel}
                </span>
              </div>
